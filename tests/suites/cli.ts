...
export function defineCliSuite(config: VersionConfig) {
  const {schemasDir, tsconfigPath} = config;

  /**
   * Compute a relative path from the CWD to the schemasDir that matches
   * the format used in generated import statements (e.g. "./tests/schemas/...").
   */
  function schemaRelPath(filename: string): string {
    const abs = path.join(schemasDir, filename);
    const rel = path.relative(process.cwd(), abs);
    // Strip the .ts extension to match import specifiers
    const withoutExt = rel.replace(/\.ts$/, '');
    return `./${withoutExt.split(path.sep).join('/')}`;
  }

  describe(`getGeneratedSchema (${config.name})`, () => {
    let tsProject: Project;
    const outputFilePath = 'test-output.gen.ts';
    const schemaPath = path.join(schemasDir, 'one-to-one.zero.ts');
    ...
    it('should handle complex schema properties correctly', () => {
      ...
      // Verify the import path references the correct schemas directory
      const expectedImportPath = schemaRelPath('one-to-one.zero.ts');
      expect(generatedSchema).toContain(`from "${expectedImportPath}";`);
    });
    ...
    it('should throw error when source file is not found', () => {
      ...
      const nonExistentPath = path.join(schemasDir, 'does-not-exist.ts');
    ...
    it('should add .js file extensions to imports when jsExtensionOverride is "force"', () => {
      ...
      const expectedImportPath = schemaRelPath('one-to-one.zero.ts');
      expect(generatedSchema).toContain(`from "${expectedImportPath}.js";`);
    ...
    it('should not add .js file extensions to imports when jsExtensionOverride is "none"', () => {
      ...
      const expectedImportPath = schemaRelPath('one-to-one.zero.ts');
      expect(generatedSchema).toContain(`from "${expectedImportPath}";`);
      expect(generatedSchema).not.toContain(`from "${expectedImportPath}.js";`);
  ...
  describe(`drizzle-kit functions (${config.name})`, () => {
    let tsProject: Project;
    const schemaPath = path.join(schemasDir, 'one-to-one.zero.ts');
  ...
  describe(`getConfigFromFile (${config.name})`, () => {
    let tsProject: Project;
  ...
  describe(`signature integration (${config.name})`, () => {
    let tsProject: Project;
    const outputFilePath = 'test-output.gen.ts';
    const schemaPath = path.join(schemasDir, 'one-to-one.zero.ts');
